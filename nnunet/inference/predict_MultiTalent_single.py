...
def preprocess_save_to_queue(preprocess_fn, list_of_list, output_files, segs_from_prev_stage, classes,
                             transpose_forward):
    # suppress output
    ...
    try:
        ...
        d, _, dct = preprocess_fn(list_of_list)
...
def preprocess_multithreaded(trainer, list_of_list, len_lists, output_file, num_processes=2, segs_from_prev_stage=None):
    if segs_from_prev_stage is None:
        segs_from_prev_stage = [None] * len_lists
    ...
    # for i in range(num_processes):
        ...
        # pr.start()
        # processes.append(pr)
    output_file, (d, dct) = preprocess_save_to_queue(trainer.preprocess_patient, 
                                    list_of_list, 
                                    output_file, 
...
def predict_cases(model, list_of_lists, output_filenames, folds, save_npz, num_threads_preprocessing,
    ...
    for i in range(len_lists):
        output_filename, (d, dct) = preprocess_multithreaded(trainer, list_of_lists[i], len_lists, cleaned_output_files[i], num_threads_preprocessing,
                                             segs_from_prev_stage)
        all_output_files.append(all_output_files)
        if isinstance(d, str): # not pass
            data = np.load(d)
            os.remove(d)
            d = data
        ...
        for l in MultiTalent_regions.keys():
            ...
            if save_npz:
                ...
                npz_name = None
            result = save_segmentation_nifti_from_softmax, ((softmax_pred_here, dr +'/individual/' + f[:-7] +'_' +region_name+ '.nii.gz',
...
def main():
    ...
    parser.add_argument("-i", '--input_folder', help="Must contain all modalities for each patient in the correct"
                                                     ...
                                                     default='/data2/wyn/nnUNetFrame/nnUNet_raw_data_base/nnUNet_raw_data/Task101_MTvessel/imagesTs/')
    parser.add_argument('-o', "--output_folder", required=False, help="folder for saving predictions",
                        default='/data2/wyn/nnUNetFrame/nnUNet_trained_models/nnUNet/3d_fullres/Task101_MTvessel/MultiTalent_trainer_ddp__MultiTalent_bs4/all/all/')
    parser.add_argument('-m', '--model_output_folder',
                        help='model output folder. Will automatically discover the folds '
                             ...
                             default='/data2/wyn/nnUNetFrame/nnUNet_trained_models/nnUNet/3d_fullres/Task101_MTvessel/MultiTalent_trainer_ddp__MultiTalent_bs4/')
    ...
    parser.add_argument("--num_threads_nifti_save", required=False, default=0, type=int, help=

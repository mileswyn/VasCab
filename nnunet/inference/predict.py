...
# import for nnUNetTrainerV3
from nnunet.training.dataloading.trainerV3_dataset_config import dataset_lab_map, dataset_modality_map, dataset_name_list
...
def predict_cases(model, list_of_lists, output_filenames, folds, save_npz, num_threads_preprocessing,
    ...
    # 仅供需要提供spacing和modality的使用
    # 301 spacing: [0.625, 0.564, 0.564]
    # 309 spacing: [0.7  , 0.438, 0.438]
    ttask = 'Task411_Crown_MRA'
    ...
    if ttask == 'Task303_CTA_KiPA':
        trainer.plans['plans_per_stage'][trainer.stage]['current_spacing'] = [0.637, 0.637, 0.637]
        trainer.normalization_schemes = 'nonCT'
    elif ttask == 'Task409_topCowCTA':
        trainer.plans['plans_per_stage'][trainer.stage]['current_spacing'] = [0.700, 0.430, 0.430]
        trainer.normalization_schemes = 'nonCT'
    elif ttask == 'Task410_topCowMRA':
        trainer.plans['plans_per_stage'][trainer.stage]['current_spacing'] = [0.600, 0.296, 0.296]
        trainer.normalization_schemes = 'nonCT'
    elif ttask == 'Task411_Crown_MRA':
        trainer.plans['plans_per_stage'][trainer.stage]['current_spacing'] = [0.500, 0.357, 0.357]
        trainer.normalization_schemes = 'nonCT'
    elif ttask == 'Task407_Parse22_pulmonary_artery':
        trainer.plans['plans_per_stage'][trainer.stage]['current_spacing'] = [1.0, 0.651, 0.651]
        trainer.normalization_schemes = 'CT'
    elif ttask == 'Task405_CTA_AIIB23trachea':
        trainer.plans['plans_per_stage'][trainer.stage]['current_spacing'] = [0.500, 0.710, 0.710]
        trainer.normalization_schemes = 'nonCT'
    elif ttask == 'Task412_Coron':
        trainer.plans['plans_per_stage'][trainer.stage]['current_spacing'] = [0.500, 0.351, 0.351]
        trainer.normalization_schemes = 'CT'
    else:
    ...
    for preprocessed in preprocessing:
        ...
        if not give_prior:
            ...
            if 'AIIB' in dct['list_of_data_files'][0].split('/')[-1]:
                dataset_id = '405'
            elif 'PA' in dct['list_of_data_files'][0].split('/')[-1]:
                dataset_id = '407'
            elif 'topcow_ct' in dct['list_of_data_files'][0].split('/')[-1]:
                dataset_id = '409'
            elif 'topcow_mr' in dct['list_of_data_files'][0].split('/')[-1]:
                dataset_id = '410'
            elif 'crown' in dct['list_of_data_files'][0].split('/')[-1]:
                dataset_id = '411'
            elif 'coron' in dct['list_of_data_files'][0].split('/')[-1]:
                dataset_id = '412'
            else:
                dataset_id = '303'
            # Assume batch size as 1
            # tgt是否并不需要？
            tgt_idx = torch.from_numpy(np.array([dataset_lab_map[dataset_id]]))
            mod_idx = torch.from_numpy(np.array([dataset_modality_map[dataset_id]]))
        ...
        try:
            softmax = trainer.predict_preprocessed_data_return_seg_and_softmax(
                ...
                mixed_precision=mixed_precision, tgt_idx=tgt_idx, mod_idx=mod_idx, output_filename=output_filename)[1]
        except:
            softmax = trainer.predict_preprocessed_data_return_seg_and_softmax(
                d, do_mirroring=do_tta, mirror_axes=trainer.data_aug_params['mirror_axes'], use_sliding_window=True,
                step_size=step_size, use_gaussian=True, all_in_gpu=all_in_gpu,
        ...
        for p in params[1:]:  # 多个网络的参数进行集成
            trainer.load_checkpoint_ram(p, False)
            softmax += trainer.predict_preprocessed_data_return_seg_and_softmax(
                d, do_mirroring=do_tta, mirror_axes=trainer.data_aug_params['mirror_axes'], use_sliding_window=True,
                step_size=step_size, use_gaussian=True, all_in_gpu=all_in_gpu,
                mixed_precision=mixed_precision, tgt_idx=tgt_idx, mod_idx=mod_idx)[1]
...
def predict_from_folder(model: str, input_folder: str, output_folder: str, folds: Union[Tuple[int], List[int]],
    ...
    if mode == "normal":
        ...
        return predict_cases(model, list_of_lists[part_id::num_parts], output_files[part_id::num_parts], folds,
                             ...
                             disable_postprocessing=disable_postprocessing, give_prior=give_prior)
    elif mode == "fast":
        if overwrite_all_in_gpu is None:
            all_in_gpu = False
        else:

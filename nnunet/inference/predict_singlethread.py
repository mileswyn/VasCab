...
def preprocess_save_to_queue(preprocess_fn, list_of_lists, output_files, segs_from_prev_stage, classes,
                             transpose_forward):
    # suppress output
...
def preprocess_single(trainer, list_of_lists, output_files, segs_from_prev_stage=None):
    ...
    for i in range(list_of_lists):
        pr = preprocess_save_to_queue, args=(trainer.preprocess_patient,
                                            list_of_lists[i],
                                            output_files[i],
...
def predict_cases(model, list_of_lists, output_filenames, folds, save_npz, num_threads_preprocessing,
    ...
    trainer.plans['plans_per_stage'][trainer.stage]['current_spacing'] = [1.0, 0.65, 0.65]
    ...
    for i in range(len(list_of_lists)):
        output_filename, (d, dct) = preprocess_save_to_queue(trainer.preprocess_patient,
                                                             list_of_lists[i],
                                                             cleaned_output_files[i],
                                                             segs_from_prev_stage[i],
                                                             classes, trainer.plans['transpose_forward'])
        ...
        if not give_prior:
            ...
            if 'hubei' in dct['list_of_data_files'][0].split('/')[-1]:
                dataset_id = '301'
            elif 'IXI' in dct['list_of_data_files'][0].split('/')[-1]:
                dataset_id = '302'
            elif 'AIIB' in dct['list_of_data_files'][0].split('/')[-1]:
                dataset_id = '405'
            elif 'subject' in dct['list_of_data_files'][0].split('/')[-1]:
                dataset_id = '406'

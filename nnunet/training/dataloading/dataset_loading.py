...
# 以下是trainerV3需要使用的依赖
from .trainerV3_dataset_config import dataset_lab_map, dataset_modality_map, dataset_name_list
import torch
import time
# np.random.seed = 42
...
class DataLoader3D(SlimDataLoaderBase):
    def __init__(self, data, patch_size, final_patch_size, batch_size, has_prev_stage=False,
                 ...
                 pad_sides=None, sampling_probabilities=None, give_prior=False):
        """
    ...
    def generate_train_batch(self):
        ...
        # selected_keys = np.array(['28', '10', 'AIIB23_148', 'topcow_ct_whole_020'])
        ...
        for j, i in enumerate(selected_keys):
            ...
            if not self.give_prior:
                pass
            else:
                # dataset_id = properties['seg_file'].split('/')[-1]
                # if 'hubei' in properties['seg_file'].split('/')[-1]:
                #     dataset_id = '301'
                # elif 'IXI' in properties['seg_file'].split('/')[-1]:
                #     dataset_id = '302'
                if 'AIIB' in properties['seg_file'].split('/')[-1]:
                    dataset_id = '405'
                # elif 'subject' in properties['seg_file'].split('/')[-1]:
                #     dataset_id = '406'
                elif 'PA' in properties['seg_file'].split('/')[-1]:
                    dataset_id = '407'
                elif 'topcow_ct' in properties['seg_file'].split('/')[-1]:
                    dataset_id = '409'
                elif 'topcow_mr' in properties['seg_file'].split('/')[-1]:
                    dataset_id = '410'
                elif 'crown' in properties['seg_file'].split('/')[-1]:
                    dataset_id = '411'
                elif 'coron' in properties['seg_file'].split('/')[-1]:
                    dataset_id = '412'
                else:
                    dataset_id = '303'
                properties['tgt'] = dataset_lab_map[dataset_id]
                properties['mod'] = dataset_modality_map[dataset_id]
            ...
            #     bigger = None
            # else:
            #     bigger = [True,True,True]
            #     for d in range(len(shape)):
            #         if shape[d] < self.patch_size[d]:
            #             bigger[d] = False
            #     bigger = np.array(bigger).astype(np.int8)
            #     # print(bigger)
            #     if np.sum(bigger) == 3:
            #         lb_x = 0
            #         ub_x = shape[0] - self.patch_size[0]
            ...
            if not force_fg:
                ...
                if voxels_of_that_class is not None:
                    ...
                    # if self.give_prior:
                    #     if selected_voxel[0] + self.patch_size[0] // 2 < shape[0]:
                    #         bbox_x_lb = max(lb_x, selected_voxel[0] - self.patch_size[0] // 2)
                    #     else:
                    #         bbox_x_lb = np.random.randint(lb_x, ub_x + 1)
                    #     if selected_voxel[1] + self.patch_size[1] // 2 < shape[1]:
                    #         bbox_y_lb = max(lb_y, selected_voxel[1] - self.patch_size[1] // 2)
                    #     else:
                    #         bbox_y_lb = np.random.randint(lb_y, ub_y + 1)
                    #     if selected_voxel[2] + self.patch_size[2] // 2 < shape[2]:
            ...
            # cptime = time.time()
            new_shape = (case_all_data.shape[0], valid_bbox_x_ub - valid_bbox_x_lb, valid_bbox_y_ub - valid_bbox_y_lb, valid_bbox_z_ub - valid_bbox_z_lb)
            padded_case_all_data = np.empty(new_shape, dtype=case_all_data.dtype)
            padded_case_all_data = case_all_data[:, valid_bbox_x_lb:valid_bbox_x_ub,
                                    valid_bbox_y_lb:valid_bbox_y_ub,
                                    valid_bbox_z_lb:valid_bbox_z_ub]
            # case_all_data = np.copy(case_all_data[:, valid_bbox_x_lb:valid_bbox_x_ub,
            #                         valid_bbox_y_lb:valid_bbox_y_ub,
            #                         valid_bbox_z_lb:valid_bbox_z_ub])
            # typetbegintime = time.time()
            ...
            # else:
            data[j] = np.pad(padded_case_all_data[:-1], ((0, 0),
                                                (-min(0, bbox_x_lb), max(bbox_x_ub - shape[0], 0)),
                                                (-min(0, bbox_y_lb), max(bbox_y_ub - shape[1], 0)),
                                                (-min(0, bbox_z_lb), max(bbox_z_ub - shape[2], 0))),
                            self.pad_mode, **self.pad_kwargs_data)
            # segtime = time.time()
            seg[j, 0] = np.pad(padded_case_all_data[-1:], ((0, 0),
                                                    (-min(0, bbox_x_lb), max(bbox_x_ub - shape[0], 0)),
                                                    (-min(0, bbox_y_lb), max(bbox_y_ub - shape[1], 0)),
                                                    (-min(0, bbox_z_lb), max(bbox_z_ub - shape[2], 0))),
                            'constant', **{'constant_values': -1})
            # print(' -- autual pad shape', ((-min(0, bbox_x_lb), max(bbox_x_ub - shape[0], 0)),
            #                                 (-min(0, bbox_y_lb), max(bbox_y_ub - shape[1], 0)),
            #                                 (-min(0, bbox_z_lb), max(bbox_z_ub - shape[2], 0))))
            # print(' -- load shape', case_all_data[:-1].shape, ' -- padded shape', self.patch_size, ' -- need to pad: ', need_to_pad)
            # print(properties['seg_file'].split('/')[-1])
            # print(' -- data trans time', time.time() - cptime)
            # print(properties['seg_file'].split('/')[-1], ' -- data copy time', datatime - cptime, ' -- seg pad time: ', time.time() - datatime)
            # print(' -- load time', loadendtime - loadbegintime)
            # print(' -- type translation time', cptime - typetbegintime)
            if seg_from_previous_stage is not None:
        ...
        # etime = time.time()
        # print('total time: ', etime - btime)
        if not self.give_prior:
            return {'data': data, 'seg': seg, 'properties': case_properties, 'keys': selected_keys}
        else:
            tgt_idx, mod_idx = [], []
            for i in range(len(case_properties)):
                # tgt_idx.append(case_properties[i]['tgt'])
                mod_idx.append(case_properties[i]['mod'])
            # tgt_idx = torch.from_numpy(np.array(tgt_idx))
            mod_idx = torch.from_numpy(np.array(mod_idx))
            # return {'data': data, 'seg': seg, 'properties': case_properties, 'keys': selected_keys, 
            #         'tgt': tgt_idx, 'mod': mod_idx}
            return {'data': data, 'seg': seg, 'properties': case_properties, 'keys': selected_keys, 'mod': mod_idx}

# class DataLoader3D_UniVessel(DataLoader3D):
#     def __init__(self, data, patch_size, final_patch_size, batch_size, has_prev_stage=False,
#                  oversample_foreground_percent=0.0, memmap_mode="r", pad_mode="edge", pad_kwargs_data=None,
#                  pad_sides=None, sampling_probabilities=None, give_prior=False):
...
#         selected_keys = np.random.choice(self.list_of_keys, self.batch_size, True, self.sampling_probabilities)
#         # selected_keys = np.array(['28', '10', 'AIIB23_148', 'topcow_ct_whole_020'])
#         data = np.zeros(self.data_shape, dtype=np.float16)
#         seg = np.zeros(self.seg_shape, dtype=np.float16)
#         case_properties = []
#         for j, i in enumerate(selected_keys):
#             # oversampling foreground will improve stability of model training, especially if many patches are empty
#             # (Lung for example)
...
#                 elif 'IXI' in properties['seg_file'].split('/')[-1]:
#                     dataset_id = '302'
#                 elif 'AIIB' in properties['seg_file'].split('/')[-1]:
#                     dataset_id = '405'
#                 elif 'subject' in properties['seg_file'].split('/')[-1]:
#                     dataset_id = '406'
#                 elif 'PA' in properties['seg_file'].split('/')[-1]:
...
#                 pass
#             elif 'AIIB' in properties['seg_file'].split('/')[-1]:
#                 # AIIB shapes at 3 dim > patch size(210, 289, 219)
#                 lb_x = 0
#                 ub_x = shape[0] - self.patch_size[0]
#                 lb_y = 0
#                 ub_y = shape[1] - self.patch_size[1]
...
#             case_all_data = case_all_data[:, valid_bbox_x_lb:valid_bbox_x_ub,
#                                     valid_bbox_y_lb:valid_bbox_y_ub,
#                                     valid_bbox_z_lb:valid_bbox_z_ub]#.astype(np.float16)
#             if seg_from_previous_stage is not None:
#                 seg_from_previous_stage = seg_from_previous_stage[:, valid_bbox_x_lb:valid_bbox_x_ub,
#                                           valid_bbox_y_lb:valid_bbox_y_ub,
#                                           valid_bbox_z_lb:valid_bbox_z_ub]
...
#                                                   (-min(0, bbox_y_lb), max(bbox_y_ub - shape[1], 0)),
#                                                   (-min(0, bbox_z_lb), max(bbox_z_ub - shape[2], 0))),
#                              self.pad_mode, **self.pad_kwargs_data)#.astype(np.float32)
#             # segtime = time.time()
#             seg[j, 0] = np.pad(case_all_data[-1:], ((0, 0),
#                                                     (-min(0, bbox_x_lb), max(bbox_x_ub - shape[0], 0)),
#                                                     (-min(0, bbox_y_lb), max(bbox_y_ub - shape[1], 0)),
#                                                     (-min(0, bbox_z_lb), max(bbox_z_ub - shape[2], 0))),
#                                'constant', **{'constant_values': -1})#.astype(np.float32)
#             # print(' -- autual pad shape', ((-min(0, bbox_x_lb), max(bbox_x_ub - shape[0], 0)),
#             #                                 (-min(0, bbox_y_lb), max(bbox_y_ub - shape[1], 0)),
#             #                                 (-min(0, bbox_z_lb), max(bbox_z_ub - shape[2], 0))))
#             # print(' -- load shape', case_all_data[:-1].shape, ' -- padded shape', self.patch_size, ' -- need to pad: ', need_to_pad)
#             # print(' -- data pad time', segtime - cptime, ' -- seg pad time: ', time.time() - segtime)
#             # print(' -- load time', loadendtime - loadbegintime)
#             # print(' -- type translation time', cptime - typetbegintime)
#             if seg_from_previous_stage is not None:
#                 seg[j, 1] = np.pad(seg_from_previous_stage, ((0, 0),

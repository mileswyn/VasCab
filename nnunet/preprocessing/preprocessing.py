...
class GenericPreprocessor(object):
    ...
    def resample_and_normalize(self, data, target_spacing, properties, seg=None, force_separate_z=None):
        ...
        for c in range(len(data)):
            ...
            scheme = self.normalization_scheme_per_modality
            if scheme == "CT":
                # clip to lb and ub from train data foreground and use foreground mn and sd from training data
                assert self.intensityproperties is not None, "ERROR: if there is a CT then we need intensity properties"

                # ### Task301 可能需要调整
                # mean_intensity = 240.92073
                # std_intensity = 131.30022
                # lower_bound = 0.0
                # upper_bound = 900.0
                
                # ## Task409
                # mean_intensity = 196.09804
                # std_intensity = 91.15319
                # lower_bound = 0.0
                # upper_bound = 600.00
                
                ### Task406
                # mean_intensity = 209.27023
                # std_intensity = 103.92418
                # lower_bound = -100.0
                # upper_bound = 1000.0
                
                # ### Task407
                # mean_intensity = 38.46
                # std_intensity = 304.216
                # lower_bound = -1000.0
                # upper_bound = 800.0
...
class GenericPreprocessor_mtvessel(GenericPreprocessor):
    ...
    def run(self, target_spacings, input_folder_with_cropped_npz, output_folder, data_identifier,
        ...
        target_spacings = np.array([[0.64, 0.35, 0.35], [0.50, 0.71, 0.71], [1.0, 1.0, 1.0], [1.0, 0.65, 0.65]])
        print("Initializing to run preprocessing for mtvessels")
        print("npz folder:", input_folder_with_cropped_npz)
        print("output_folder:", output_folder)
        list_of_cropped_npz_files = subfiles(input_folder_with_cropped_npz, True, None, ".npz", True)
        maybe_mkdir_p(output_folder)
        ...
        all_args = []
        output_folder_stage = os.path.join(output_folder, data_identifier + "_stage1")
        maybe_mkdir_p(output_folder_stage)
    ...
    def _run_internal(self, target_spacing, case_identifier, output_folder_stage, cropped_output_dir, force_separate_z,
                      all_classes):
        if 'AIIB' in case_identifier:
            target_spacing = target_spacing[1]
        elif 'subject' in case_identifier:
            target_spacing = target_spacing[2]
        elif 'PA' in case_identifier:
            target_spacing = target_spacing[3]
        else:
            target_spacing = target_spacing[0]

...
#    用于研究所有任务融合时，如何对综合性的任务构建网络的plan（仅在debug模式下试行）

import nnunet
from batchgenerators.utilities.file_and_folder_operations import *
from nnunet.experiment_planning.AllDatasetAnalyzer import DatasetAnalyzer
...
def main():
    ...
    #                                                         "data folder")
    parser.add_argument("-t", "--task_ids", type=str, default=['301','302','303','405','406','407','409','410','411'], 
    # parser.add_argument("-t", "--task_ids", type=str, default=['411'], 
                        help="List of integers belonging to the task ids you wish to run"
                             " experiment planning and preprocessing for. Each of these "
                             "ids must, have a matching folder 'TaskXXX_' in the raw "
                             "data folder")
    parser.add_argument("-pl3d", "--planner3d", type=str, default="ExperimentPlanner3D_v21_all_vessel",
                        help="Name of the ExperimentPlanner class for the full resolution 3D U-Net and U-Net cascade. "
                             ...
                             "configured")
    # parser.add_argument("-pl3d", "--planner3d", type=str, default="ExperimentPlanner3D_v21_MultiTalent",
    #                     help="Name of the ExperimentPlanner class for the full resolution 3D U-Net and U-Net cascade. "
    #                          "Default is ExperimentPlanner3D_v21. Can be 'None', in which case these U-Nets will not be "
    #                          "configured")
    ...
    # parser.add_argument("-overwrite_plans_identifier", type=str, default='multitalent_bs4', required=False,
    ...
    # nnunet.__path__[0] = '/data2/wyn/MultiTalent/nnunet'
    ...
    # overwrite: calculate all dataset info, save it temporally in Task301.
    cropped_out_dir = []
    preprocessing_output_dir_all = os.path.join(preprocessing_output_dir, 'all_vessel_task')
    for t in tasks:
        cropped_out_dir.append(os.path.join(nnUNet_cropped_data, t))
    dataset_analyzer = DatasetAnalyzer(cropped_out_dir, overwrite=False, num_processes=1)
    _ = dataset_analyzer.analyze_dataset(collect_intensityproperties=False)

    maybe_mkdir_p(preprocessing_output_dir_all)
    shutil.copy(join(cropped_out_dir[0], "dataset_properties_all_vessel.pkl"), preprocessing_output_dir_all)
    # shutil.copy(join(nnUNet_raw_data, t, "dataset.json"), preprocessing_output_dir_this_task)
    exp_planner = planner_3d(cropped_out_dir, preprocessing_output_dir_all)
    ...
    for t in tasks:
        ...
        shutil.copy(join(cropped_out_dir, "dataset_properties_all.pkl"), preprocessing_output_dir_this_task)
        shutil.copy(join(nnUNet_raw_data, t, "dataset.json"), preprocessing_output_dir_this_task)

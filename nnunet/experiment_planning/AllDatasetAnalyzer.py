...
class DatasetAnalyzer(object):
    def __init__(self, folder_with_cropped_data, overwrite=True, num_processes=default_num_threads):
        ...
        self.patient_identifiers = [get_patient_identifiers_from_cropped_files(i) for i in self.folder_with_cropped_data]
        # assert isfile(join(self.folder_with_cropped_data, "dataset.json")), \
        #     "dataset.json needs to be in folder_with_cropped_data"
        self.props_per_case_file = [join(i, "props_per_case.pkl") for i in self.folder_with_cropped_data]
        self.intensityproperties_file = [join(i, "intensityproperties.pkl") for i in self.folder_with_cropped_data]

    def load_properties_of_cropped(self, case_identifier, num_task):
        with open(join(self.folder_with_cropped_data[num_task], "%s.pkl" % case_identifier), 'rb') as f:
    ...
    def get_classes(self):
        datasetjson = load_json(join(self.folder_with_cropped_data[0], "dataset.json"))
    ...
    def get_sizes_and_spacings_after_cropping(self):
        ...
        for num, t in enumerate(self.patient_identifiers):
            for c in self.patient_identifiers[num]:
                properties = self.load_properties_of_cropped(c, num)
    ...
    def get_size_reduction_by_cropping(self):
        ...
        for num, t in enumerate(self.patient_identifiers):
            for p in self.patient_identifiers[num]:
                props = self.load_properties_of_cropped(p, num)
    ...
    def analyze_dataset(self, collect_intensityproperties=True):
        ...
        save_pickle(dataset_properties, join(self.folder_with_cropped_data[0], "dataset_properties_all_vessel.pkl"))

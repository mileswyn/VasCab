...
class HiLo(nn.Module):
    ...
    def hifi(self, x):
        B, D, W, H, C = x.shape
        d_group, w_group, h_group = D // self.ws, W // self.ws, H // self.ws

        total_groups = h_group * w_group * d_group

        # x = x.reshape(B, h_group, self.ws, w_group, self.ws, C).transpose(2, 3)
        x = x.reshape(B, d_group, w_group, h_group, self.ws, self.ws, self.ws, C)
    ...
    def lofi(self, x):
        ...
        q = self.l_q(x).reshape(B, D * H * W, self.l_heads, self.l_dim // self.l_heads).permute(0, 2, 1, 3)
    ...
    def forward(self, x, D, W, H):
        B, N, C = x.shape

        x = x.reshape(B, D, W, H, C)
...
if __name__ == "__main__":
    m = HiLo(dim=128)
    hr_feat = torch.rand(2, 38400, 128)
    # lr_feat = torch.rand(1, 128, 16, 16, 16)
    feat = m(hr_feat, 24, 40, 40)
    # print(hr_feat.size(), lr_feat.size())
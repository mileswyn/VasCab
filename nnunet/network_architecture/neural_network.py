...
class SegmentationNetwork(NeuralNetwork):
    ...
    def predict_3D(self, x: np.ndarray, do_mirroring: bool, mirror_axes: Tuple[int, ...] = (0, 1, 2),
                   ...
                   verbose: bool = True, mixed_precision: bool = True, region_vec = None,
                ...
                if self.conv_op == nn.Conv3d:
                    if use_sliding_window:
                        res = self._internal_predict_3D_3Dconv_tiled(x, step_size, do_mirroring, mirror_axes, patch_size,
                                                                     ...
                                                                     verbose=verbose, region_vec=region_vec, tgt_idx=tgt_idx, 
                                                                     mod_idx=mod_idx, output_filename=output_filename)
                    else:
                        res = self._internal_predict_3D_3Dconv(x, patch_size, do_mirroring, mirror_axes, regions_class_order,
                                                               ...
                                                               tgt_idx=tgt_idx, mod_idx=mod_idx)
    ...
    def _internal_predict_3D_3Dconv_tiled(self, x: np.ndarray, step_size: float, do_mirroring: bool, mirror_axes: tuple,
                                          ...
                                          verbose: bool, region_vec=None, tgt_idx=None, mod_idx=None, output_filename=None) -> Tuple[np.ndarray, np.ndarray]:
        ...
        # for x in steps[0]:
        for x_id, x in enumerate(steps[0]):
            lb_x = x
            ub_x = x + patch_size[0]
            # for y in steps[1]:
            for y_id, y in enumerate(steps[1]):
                lb_y = y
                ub_y = y + patch_size[1]
                # for z in steps[2]:
                for z_id, z in enumerate(steps[2]):
                    lb_z = z
                    ub_z = z + patch_size[2]

                    predicted_patch = self._internal_maybe_mirror_and_pred_3D(
                        data[None, :, lb_x:ub_x, lb_y:ub_y, lb_z:ub_z], mirror_axes, do_mirroring,
                        gaussian_importance_map, region_vec=region_vec, tgt_idx=tgt_idx, mod_idx=mod_idx)[0]
                    
                    if x_id == len(steps[0]) // 2 and y_id == len(steps[1]) // 2 and z_id == len(steps[2]) // 2:
                        predicted_middle_npy = predicted_patch.detach().cpu().numpy()
                        np.save(os.path.join('/data2/wyn/MultiTalent/tsne_npy_better/411/', output_filename.split('/')[-1].split('.nii')[0]+'.npy'), predicted_middle_npy)
    ...
    def _internal_predict_3D_3Dconv(self, x: np.ndarray, min_size: Tuple[int, ...], do_mirroring: bool,
                                    mirror_axes: tuple = (0, 1, 2), regions_class_order: tuple = None,
                                    pad_border_mode: str = "constant", pad_kwargs: dict = None,
                                    verbose: bool = True, region_vec = None, tgt_idx = None, 
                                    mod_idx = None) -> Tuple[np.ndarray, np.ndarray]:
        """
        This one does fully convolutional inference. No sliding window
        """
        assert len(x.shape) == 4, "x must be (c, x, y, z)"
        ...
        predicted_probabilities = self._internal_maybe_mirror_and_pred_3D(data[None], mirror_axes, do_mirroring,
                                                                          None, region_vec, tgt_idx, mod_idx)[0]
    ...
    def _internal_maybe_mirror_and_pred_3D(self, x: Union[np.ndarray, torch.tensor], mirror_axes: tuple,
                                           ...
                                           mult: np.ndarray or torch.tensor = None, region_vec = None,
                                           tgt_idx = None, mod_idx = None) -> torch.tensor:
        ...
        for m in range(mirror_idx):
            if m == 0:
                if region_vec is not None:
                    ...
                    # if tgt_idx is not None and mod_idx is not None:
                    #     pred, mod_pred = self(x, tgt_idx, mod_idx.unsqueeze(1))
                    #     pred = self.inference_apply_nonlin(pred)
                    if mod_idx is not None:
                        pred, mod_pred = self(x, mod_idx.unsqueeze(1))
                        pred = self.inference_apply_nonlin(pred)
                    else:
            ...
            if m == 1 and (2 in mirror_axes):
                if region_vec is not None:
                    ...
                    # if tgt_idx is not None and mod_idx is not None:
                    #     pred, mod_pred = self(torch.flip(x, (4, )), tgt_idx, mod_idx.unsqueeze(1))
                    #     pred = self.inference_apply_nonlin(pred)
                    if tgt_idx is not None and mod_idx is not None:
                        pred, mod_pred = self(torch.flip(x, (4, )), mod_idx.unsqueeze(1))
                        pred = self.inference_apply_nonlin(pred)
                    else:
            ...
            if m == 2 and (1 in mirror_axes):
                if region_vec is not None:
                    ...
                    # if tgt_idx is not None and mod_idx is not None:
                    #     pred, mod_pred = self(torch.flip(x, (3, )), tgt_idx, mod_idx.unsqueeze(1))
                    #     pred = self.inference_apply_nonlin(pred)
                    if tgt_idx is not None and mod_idx is not None:
                        pred, mod_pred = self(torch.flip(x, (3, )), mod_idx.unsqueeze(1))
                        pred = self.inference_apply_nonlin(pred)
                    else:
            ...
            if m == 3 and (2 in mirror_axes) and (1 in mirror_axes):
                if region_vec is not None:
                    ...
                    # if tgt_idx is not None and mod_idx is not None:
                    #     pred, mod_pred = self(torch.flip(x, (4, 3)), tgt_idx, mod_idx.unsqueeze(1))
                    #     pred = self.inference_apply_nonlin(pred)
                    if tgt_idx is not None and mod_idx is not None:
                        pred, mod_pred = self(torch.flip(x, (4, 3)), mod_idx.unsqueeze(1))
                        pred = self.inference_apply_nonlin(pred)
                    else:
            ...
            if m == 4 and (0 in mirror_axes):
                if region_vec is not None:
                    ...
                    # if tgt_idx is not None and mod_idx is not None:
                    #     pred, mod_pred = self(torch.flip(x, (2, )), tgt_idx, mod_idx.unsqueeze(1))
                    #     pred = self.inference_apply_nonlin(pred)
                    if tgt_idx is not None and mod_idx is not None:
                        pred, mod_pred = self(torch.flip(x, (2, )), mod_idx.unsqueeze(1))
                        pred = self.inference_apply_nonlin(pred)
                    else:
            ...
            if m == 5 and (0 in mirror_axes) and (2 in mirror_axes):
                if region_vec is not None:
                    ...
                    # if tgt_idx is not None and mod_idx is not None:
                    #     pred, mod_pred = self(torch.flip(x, (4, 2)), tgt_idx, mod_idx.unsqueeze(1))
                    #     pred = self.inference_apply_nonlin(pred)
                    if tgt_idx is not None and mod_idx is not None:
                        pred, mod_pred = self(torch.flip(x, (4, 2)), mod_idx.unsqueeze(1))
                        pred = self.inference_apply_nonlin(pred)
                    else:
            ...
            if m == 6 and (0 in mirror_axes) and (1 in mirror_axes):
                if region_vec is not None:
                    ...
                    # if tgt_idx is not None and mod_idx is not None:
                    #     pred, mod_pred = self(torch.flip(x, (3, 2)), tgt_idx, mod_idx.unsqueeze(1))
                    #     pred = self.inference_apply_nonlin(pred)
                    if tgt_idx is not None and mod_idx is not None:
                        pred, mod_pred = self(torch.flip(x, (3, 2)), mod_idx.unsqueeze(1))
                        pred = self.inference_apply_nonlin(pred)
                    else:
            ...
            if m == 7 and (0 in mirror_axes) and (1 in mirror_axes) and (2 in mirror_axes):
                if region_vec is not None:
                    ...
                    # if tgt_idx is not None and mod_idx is not None:
                    #     pred, mod_pred = self(torch.flip(x, (4, 3, 2)), tgt_idx, mod_idx.unsqueeze(1))
                    #     pred = self.inference_apply_nonlin(pred)
                    if tgt_idx is not None and mod_idx is not None:
                        pred, mod_pred = self(torch.flip(x, (4, 3, 2)), mod_idx.unsqueeze(1))
                        pred = self.inference_apply_nonlin(pred)
                    else:
